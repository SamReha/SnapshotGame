...
namespace AssemblyCSharp {
	public class SpacingHeuristics {
		...
		public float bottomThird(GameObject subject, List<GameObject> visibleObjects, Camera cam) {
			List<float> rowDensity = new List<float>(new float[3]);

			// The value of sectorBoundaries is a quartet of floats such that:
			// value[0] and value[1] are the min and max x values
			// value[2] and value[3] are the min and max y values
			Dictionary<int, float> rowBoundaries = new Dictionary<int, float> ();
			rowBoundaries.Add (0, 0f, 0.333f);
			rowBoundaries.Add (1, 0.333f, 0.666f);
			rowBoundaries.Add (1, 0.666f, 1f);
		...
		public float hotSpots(GameObject subject, List<GameObject> visibleObjects, Camera cam) {
